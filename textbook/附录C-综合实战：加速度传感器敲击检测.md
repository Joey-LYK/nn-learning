...
main 里喂的是一个高 0.2g、σ=3 的窄高斯鼓包（模拟误触），**预期输出**——和正文误触样例的特征形态一致（峰值小、陡峭度小、过半样本多）：
...
然后是本附录的主角：约 90 行的 C 推理程序。它不做训练、不求梯度，只做三件事——读模型、读样本、前向传播：
...
3. **前向推理**：归一化（用模型文件里的统计量）→ 两层前向 → argmax。整个网络只有 83 个参数、**72 次乘加**（6×8 + 8×3），对现代单片机是几微秒的事
...
- 部署 = 归一化统计量 + 权重 + 激活类型的完整交付（全部信息 1.1 KB），C 端前向只有 72 次乘加
...
不过还留了一问给你带走：同一份 `tap_features.csv`，换一群"没有梯度"的算法来考，成绩会怎样？附录D 把决策树、朴素贝叶斯、kNN 们请上了同一个考场——顺带回答一个常被问起的问题：神经网络到底吃不吃数据。
